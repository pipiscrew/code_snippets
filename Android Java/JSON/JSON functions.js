//my
	...
	public static List<JSONArray> getStringListArrFromJsonArray(JSONArray jArray) throws JSONException {
		List<JSONArray> returnList = new ArrayList<JSONArray>();
		for (int i = 0; i < jArray.length(); i++) {
			returnList.add( jArray.getJSONArray(i));
		}
		return returnList;
	}
...
//sample of use 
					JSONArray valArray =  jsonObj.getJSONObject(nameArray.getString(0)).getJSONArray("items");

					//get the list of objects!
					List<JSONObject> kk = getStringListOBJFromJsonArray(valArray);
					
					String items="";

					//****************************************-1 because trying to download the PDF!
					for(int i=0;i<kk.size()-1;i++)
					{
						//for each item extract name + html 
						JSONObject  desc = kk.get(i);
						
						//items +=  desc.getString("name") + " // " + desc.getString("html");
						items += "<h2>" + desc.getString("name") + "</h2><br>" + desc.getString("html"); 
//						String elem0a = desc.getString("name");
//						items +=elem0a;
//						String elem1a = desc.getString("html");
//						items +=elem1a;
...
//original
//http://www.java2s.com/Code/Android/Development/GetListFromJsonObject.htm
